...
from .rust_adapter import extract_elements


class TreeSitterRustExtractor(TreeSitterExtractorBase):
    def __init__(self, enable_performance_monitoring: bool = True):
        super().__init__(language_id="rust", enable_performance_monitoring=enable_performance_monitoring)
    ...
    def extract_from_directory(self, directory: Path) -> List[CodeElement]:
        ...
        logging.info(f"Found {len(filtered_files)} Rust files to analyze (after filtering .gitignore).")
